...
static void send_xgear_info(void) {
  ...
  DOWNLINK_SEND_XGEAR_INFO(DefaultChannel, DefaultDevice,
      &xgear_tx.msg_cnt,
      &tx_sec_cnt,
      &xgear_rx.msg_cnt,
      &rx_sec_cnt,
...
/**
 * Initialize varoables
 * and if needed start the thread
 */
void xgear_init(void)
{
  ...
  chEvtInit(&eventXgearRx);

  xgear_flag_tx = 0;

  chMtxInit(&xgear_rx_mutex_flag);
  chThdCreateStatic(wa_thd_xgear_rx, sizeof(wa_thd_xgear_rx), HIGHPRIO - 1,
      thd_xgear_rx, NULL);

#if DOWNLINK
  register_periodic_telemetry(DefaultPeriodic, "XGEAR_INFO", send_xgear_info);
#endif
}
...
void xgear_parse(uint8_t c)
{
  switch (xgear_rx.status) {
    ...
    case MsgSize:  // SIZE depends on TYPE, idx = 3,4
      ...
      xgear_rx.idx++;
      if (xgear_rx.idx == 5) {
        xgear_rx.datalength = (uint16_t) (xgear_rx.msg_buf[xgear_rx.idx - 2]
            << 8 | xgear_rx.msg_buf[xgear_rx.idx - 1]);
    ...
    case MsgHdrChksum:  // HDR CHKSUM, idx=5,6
      ...
      xgear_rx.idx++;
      if (xgear_rx.idx == 7) {
        xgear_rx.rx_chksum = (xgear_rx.msg_buf[xgear_rx.idx - 2] << 8)
            + xgear_rx.msg_buf[xgear_rx.idx - 1];
        xgear_rx.calc_chksum = calculate_checksum(xgear_rx.msg_buf + 2,
    ...
    case MsgData:  // Read data
      ...
      if (xgear_rx.idx == (xgear_rx.datalength + XGEAR_CHK_LEN)) {
        xgear_rx.rx_chksum = (xgear_rx.msg_buf[xgear_rx.idx - 2] << 8)
            + xgear_rx.msg_buf[xgear_rx.idx - 1];
        xgear_rx.calc_chksum = calculate_checksum(xgear_rx.msg_buf,
            xgear_rx.datalength);
        if (xgear_rx.rx_chksum == xgear_rx.calc_chksum) {
...
void xgear_send_sysid(void)
{
  ...
  // timestamp
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.timestamp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // YawPitchRoll [deg]
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.ypr, 3 * sizeof(float));
  xgear_tx.idx += 3 * sizeof(float);

  // AngularRate
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &imuf.gyro, 3 * sizeof(float));
  xgear_tx.idx += 3 * sizeof(float);

  // Position LLA
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.pos_lla,
      3 * sizeof(double));
  xgear_tx.idx += 3 * sizeof(double);

  // Velocity NED
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.vel_ned, 3 * sizeof(float));
  xgear_tx.idx += 3 * sizeof(float);

  // Accel
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &imuf.accel, 3 * sizeof(float));
  xgear_tx.idx += 3 * sizeof(float);
  ...
  //posU, float[3]
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.posU, 3 * sizeof(float));
  xgear_tx.idx += 3 * sizeof(float);

  //velU, float
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.velU, sizeof(float));
  xgear_tx.idx += sizeof(float);

  //linear acceleration imu-body frame, float [m/s^2]
  // Body acceleration without gravity
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.lin_accel,
      3 * sizeof(float));
  xgear_tx.idx += 3 * sizeof(float);

  //YprU, float[3]
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.YprU, 3 * sizeof(float));
  xgear_tx.idx += 3 * sizeof(float);

  //instatus, uint16
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.ins_status,
      3 * sizeof(uint16_t));
  xgear_tx.idx += sizeof(uint16_t);

  //Vel body, float [m/s]
  // The estimated velocity in the body (i.e. imu) frame, given in m/s.
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &ins_impl.vel_body,
      3 * sizeof(float));
  xgear_tx.idx += 3 * sizeof(float);
  ...
  // Command THROTTLE
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &commands[COMMAND_THROTTLE],
      sizeof(pprz_t));
  xgear_tx.idx += sizeof(pprz_t);

  // command ROLL
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &commands[COMMAND_ROLL],
      sizeof(pprz_t));
  xgear_tx.idx += sizeof(pprz_t);

  // command PITCH
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &commands[COMMAND_PITCH],
      sizeof(pprz_t));
  xgear_tx.idx += sizeof(pprz_t);

  // command YAW
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &commands[COMMAND_YAW],
      sizeof(pprz_t));
  xgear_tx.idx += sizeof(pprz_t);
  ...
  dta_chksum = calculate_checksum(&(xgear_tx.msg_buf[XGEAR_HEADER_LENGTH]),
      (uint16_t) XGEAR_SYSID_PAYLOAD_LENGTH);
...
void xgear_send_ctrlsys(void)
{
  ...
  xgear_tx.msg_buf[0] = XGEAR_SYNC0;
  xgear_tx.msg_buf[1] = XGEAR_SYNC1;
  xgear_tx.msg_buf[2] = XGEAR_MSG_TYPE_CONTROL_SYS;
  ...
  // ref_phi [deg]
  sp = DegOfRad(stab_att_ref_euler.phi);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // ref_theta [deg]
  sp = DegOfRad(stab_att_ref_euler.theta);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // ref_psi [deg]
  sp = DegOfRad(stab_att_ref_euler.psi);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // ref_p [deg/s]
  sp = DegOfRad(stab_att_ref_rate.p);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // ref_q [deg/s]
  sp = DegOfRad(stab_att_ref_rate.q);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // ref_r [deg/s]
  sp = DegOfRad(stab_att_ref_rate.r);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // ref_pd [deg/s^2]
  sp = DegOfRad(stab_att_ref_accel.p);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // ref_qd [deg/s^2]
  sp = DegOfRad(stab_att_ref_accel.q);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);

  // ref_rd [deg/s^2]
  sp = DegOfRad(stab_att_ref_accel.r);
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &sp, sizeof(float));
  xgear_tx.idx += sizeof(float);
  ...
  cmd = guidance_h_pos_sp.x;
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &cmd, sizeof(int32_t));
  xgear_tx.idx += sizeof(int32_t);

  // sp_y [meters (x 0.0039063)]
  cmd = guidance_h_pos_sp.y;
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &cmd, sizeof(int32_t));
  xgear_tx.idx += sizeof(int32_t);

  // ref_x [meters (x 0.0039063)]
  cmd = guidance_h_pos_ref.x;
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &cmd, sizeof(int32_t));
  xgear_tx.idx += sizeof(int32_t);

  // ref_y [meters (x 0.0039063)]
  cmd = guidance_h_pos_ref.y;
  memcpy(&xgear_tx.msg_buf[xgear_tx.idx], &cmd, sizeof(int32_t));
  xgear_tx.idx += sizeof(int32_t);
