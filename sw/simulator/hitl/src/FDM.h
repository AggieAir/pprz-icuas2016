...
public:
  ...
  void feed_jsbsim(double throttle, double aileron, double elevator, double rudder)
  {
    ...
    double inv_elevator = -1*elevator;
    FCS->SetDeCmd(inv_elevator);
    FCS->SetDrCmd(rudder);
...
//#endif
    ...
    /*
    cout << "fdm.body_ecef_vel.z = " << fdm.body_ecef_vel.z <<
        ", fdm.ltp_ecef_vel.z = " << fdm.ltp_ecef_vel.z << endl;
*/
    high_sim_rate_ = false;
...
/*
    if (!high_sim_rate_ & already_launched) {
      ...
      if (timeDiff < (fdm.init_dt*2)) {
        num_steps = num_steps - (timeDiff/fdm.curr_dt); // make sure there is at least one step
      }
      else {
        if (timeDiff > (fdm.init_dt*2)){
          //num_steps = num_steps + (timeDiff/fdm.curr_dt);
        }
          }
    ...
    */
    ...
    double timeDiff = realTime - simTime;
    /*
     * timeDiff < 0 ==> simTime ahead of the realTime -> bad
     * timeDiff > 0 ==> realTime ahead ot the Simulation time -> better
     */
    ...
    /*
     * Experimental sim checker
     */
    /*
    realTime = LogTime::getTimeSinceStartDouble(initial_time_);
    simTime = FDMExec_->GetPropertyManager()->GetNode("simulation/sim-time-sec")->getDoubleValue();
    if ((realTime - simTime) > 0.05) {
      while((realTime - simTime) > 0.05){
        FDMExec_->Run();
        simTime = FDMExec_->GetPropertyManager()->GetNode("simulation/sim-time-sec")->getDoubleValue();
      }
    }
   ...
   */
  /*
  FGPropulsion* FGProp =  FDMExec_->GetPropulsion();
  FGEngine* FGEng = FGProp->GetEngine(0);
  ...
  */
  ...
  VectorNavData fetch_data(){
    ...
    data.AngularRate[0] = (float)fdm.body_ecef_rotvel.p; // TODO!
    ...
    data.LinearAccelBody[0] = (float)propagate->GetVel(1); // TODO!
    ...
    std::cout << "Sim time: " << boost::format("%1$.5f") % sim_time << "[s], Quat0: " <<
        ...
        boost::format("%1$.5f") % jsb_quat.Entry(2) <<
        ", Quat2: " << boost::format("%1$.5f") % jsb_quat.Entry(3) <<
        ", Quat3: " << boost::format("%1$.5f") % jsb_quat.Entry(4);
     */
    /*
    std::cout << "Sim time: " << boost::format("%1$.5f") % sim_time << "[s], Yaw: " <<
        boost::format("%1$.5f") % data.YawPitchRoll[0] << ", Pitch: " <<
        boost::format("%1$.5f") % data.YawPitchRoll[1] <<
        ", Roll: " << boost::format("%1$.5f") % data.YawPitchRoll[2];
*/
    /*
    cout << "Time: " << sim_time << "[s], ";
    cout << "Lat: " << data.Position[0] << "[deg], ";
    cout << "Lon: " << data.Position[1] << "[deg], ";
    cout << "Alt: " << data.Position[2] << "[m]\n";
     */
    //cout << ", Alt: " << data.Position[2] << "[m]" << endl;
  ...
  void jsbsimvec_to_vec(DoubleVect3* fdm_vector, const FGColumnVector3* jsb_vector) {
    ...
    //cout << ">>>>>>>>> fdm_vec ="  << fdm_vector->z << ", jsb_vector = " << MetersOfFeet(jsb_vector->Entry(3)) << endl;
  }
